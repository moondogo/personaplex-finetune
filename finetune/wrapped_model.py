...
def get_fsdp_model(
    ...
    Notes:
        ...
        - PersonaPlex: 确保 dep_q=16（从 config.json 自动读取，PersonaPlex 官方权重已设为 16）。
    """
    ...
    # ── PersonaPlex: 确认 dep_q=16 ──────────────────────────────────
    # PersonaPlex 官方权重的 config.json 中 dep_q=16。
    # 标准 Moshi 是 dep_q=8。此处确认加载正确。
    actual_dep_q = getattr(model, 'dep_q', None)
    if actual_dep_q is not None:
        logger.info(f"LMModel dep_q={actual_dep_q} (expected 16 for PersonaPlex, 8 for standard Moshi)")
