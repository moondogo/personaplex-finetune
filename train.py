...
def _train(args: TrainArgs, exit_stack: ExitStack):
    ...
    interleaver = Interleaver(
        ...
        model.zero_token_id,
        keep_main_only=True,
        system_prompt_enable=args.system_prompt.enable,       # 是否启用 Hybrid System Prompt
        silence_duration_sec=args.system_prompt.silence_duration_sec,  # 静音缓冲时长
        voice_prompt_dir=args.system_prompt.voice_prompt_dir,          # voice prompt 目录
        default_text_prompt=args.system_prompt.default_text_prompt,    # 默认角色描述
    )
    ...
    while state.step < args.max_steps:
        ...
        for i in range(args.num_microbatches):
            ...
            if batch.system_prompt_len is not None:
                ...
                prompt_mask = torch.ones(B, T, device=codes.device, dtype=torch.bool)
                for b in range(B):
                    plen = batch.system_prompt_len[b].item()
                    if plen > 0:
                        prompt_mask[b, :plen] = False

            audio_mask = output.mask
            text_mask_ = output.text_mask
            if prompt_mask is not None:
                # prompt_mask: [B, T] → 扩展为 [B, 1, T]（text）和 [B, dep_q, T]（audio）
                audio_mask = output.mask & prompt_mask[:, None, :]
                text_mask_ = output.text_mask & prompt_mask[:, None, :]
            ...
            # ── PersonaPlex: text_loss * text_loss_weight 补偿 audio loss 主导 ─
            # 原版 mb_loss = text_loss + audio_loss，导致 audio loss 在数值上
            # 是 text loss 的 40-100 倍。引入 text_loss_weight 进行补偿。
